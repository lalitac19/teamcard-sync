...
const VatSelect = ({ value, onChange }: { value?: string; onChange: (v: string) => void }) => (
  <Select value={value} onValueChange={onChange}>
          ...
          <Table>
            ...
            <TableBody>
              {rows.map((r) => {
                ...
                const ready = !!r.debitAccount && !!r.vatRate;
                return (
                  <TableRow key={r.id} data-state={r.selected ? "selected" : undefined}>
                    ...
                    <TableCell><AccountSelect value={r.debitAccount} onChange={(v) => update(r.id, { debitAccount: v })} /></TableCell>
...
function ReimbursementsTab() {
  ...
  const [rows, setRows] = useState(
    approved.map((r) => ({ ...r, selected: false, debitAccount: undefined as string | undefined, vatRate: undefined as string | undefined })),
  );
  const selectedCount = rows.filter((r) => r.selected).length;
  const toggleAll = (v: boolean) => setRows(rows.map((r) => ({ ...r, selected: v })));
  const update = (id: string, patch: Partial<typeof rows[number]>) =>
...
function InvoicesTab() {
  ...
  const [rows, setRows] = useState(
    approved.map((i) => ({ ...i, selected: false, debitAccount: undefined as string | undefined, vatRate: undefined as string | undefined })),
  );
  const selectedCount = rows.filter((r) => r.selected).length;
  const toggleAll = (v: boolean) => setRows(rows.map((r) => ({ ...r, selected: v })));
  const update = (id: string, patch: Partial<typeof rows[number]>) =>
...
function TopUpsTab() {
  const [rows, setRows] = useState(
    walletTopUps.map((w) => ({ ...w, selected: false, debitAccount: undefined as string | undefined })),
  );
  const selectedCount = rows.filter((r) => r.selected).length;
  const toggleAll = (v: boolean) => setRows(rows.map((r) => ({ ...r, selected: v })));
  const update = (id: string, patch: Partial<typeof rows[number]>) =>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>Map to account</TableHead>
                <TableHead className="text-right">Amount</TableHead>
                <TableHead>Status</TableHead>
              </TableRow>
            </TableHeader>
