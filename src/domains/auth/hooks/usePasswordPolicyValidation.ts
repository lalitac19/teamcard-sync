const usePasswordPolicyValidation = (passwordPolicyArray: any) => {
    const passwordPolicy =
        Array.isArray(passwordPolicyArray) && passwordPolicyArray.length > 0
            ? passwordPolicyArray[0]
            : {};
    const validatePassword = (password: any) => {
        const errors = [];
        const minLength = passwordPolicy.minLength || 8;
        const { enableBannedPasswords, customBannedPasswords } = passwordPolicy;
