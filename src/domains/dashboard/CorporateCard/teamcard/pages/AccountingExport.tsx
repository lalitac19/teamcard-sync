    ...
    >
      <Tabs defaultValue="cards" className="w-full">
        <TabsList className="grid w-full grid-cols-2 md:max-w-xl">
          ...
          <TabsTrigger value="topups">Fund Account entries</TabsTrigger>
        </TabsList>
...
function TopUpsTab() {
  ...
  const handleExport = () => {
    if (selectedCount === 0) return;
    setRows(rows.map((r) => (r.selected && !r.exported ? { ...r, exported: true, selected: false } : r)));
    toast.success(`Exported ${selectedCount} funds as bank transfers to QuickBooks`);
  };
