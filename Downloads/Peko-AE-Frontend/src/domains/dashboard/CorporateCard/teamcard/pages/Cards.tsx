...
import { Textarea } from "@src/domains/dashboard/CorporateCard/teamcard/components/ui/textarea";
import { Switch } from "@src/domains/dashboard/CorporateCard/teamcard/components/ui/switch";
...
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@src/domains/dashboard/CorporateCard/teamcard/components/ui/tabs";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
import { cards, formatCurrency, hasCompletedFirstTopUp, memberById, members, walletAvailable, corporateDeposit, type Card as CardModel } from "@src/domains/dashboard/CorporateCard/teamcard/lib/mockData";
...
const Cards = () => {
  ...
  return (
    <AppLayout
      title="Cards"
      subtitle={`${cards.length} cards issued · ${formatCurrency(available)} available in the shared Corporate Account pool.`}
      actions={
          ...
          <Dialog>
            <DialogTrigger asChild>
              <Button
                ...
                title={!hasCompletedFirstTopUp() ? "Complete your first Corporate Account fund to issue cards" : undefined}
              >
...
function IssueCardDialog() {
  ...
  // Each card's spending limit may be set up to the full Corporate Credit Limit
  // (limits are not pooled). If the limit later contracts at cycle-end due to
  // unpaid bills, all card limits are automatically reduced to match.
  const perCardMax = corporateDeposit;
  const exceedsCorporateLimit = requested > perCardMax;
  ...
  const submit = () => {
    if (!firstTopUpDone) return toast.error("Complete your first Corporate Account fund before issuing cards");
    if (!requested || requested <= 0) return toast.error("Enter a spending limit for this card");
    if (exceedsCorporateLimit) {
      return toast.error(`Spending limit cannot exceed the Corporate Credit Limit (${formatCurrency(perCardMax)})`);
    }
    if (perTxnExceedsSpend) {
      return toast.error("Per-transaction limit cannot exceed the spending limit");
    }
    if (atmExceedsCap) {
      return toast.error(`Daily ATM limit cannot exceed 20% of the daily spending limit (${formatCurrency(atmDailyCap)})`);
    }
    toast.success(
      `Card issued · ${formatCurrency(requested)} limit` +
  ...
  return (
    <DialogContent className="sm:max-w-md max-h-[85vh] overflow-y-auto">
      <DialogHeader>
        ...
        <DialogDescription>
          Issue as many cards as you need. Each card's spending limit can be set up to your full Corporate Credit Limit ({formatCurrency(corporateDeposit)}); limits are not pooled. If unpaid bills shrink the Corporate Credit Limit at cycle close, every card's limit is automatically reduced to match.
        </DialogDescription>
      </DialogHeader>
      <div className="space-y-4 py-2">
        {!firstTopUpDone && (
          <div className="rounded-md border border-destructive/40 bg-destructive/10 p-3 text-xs text-destructive">
            You must complete your first Corporate Account fund before any card can be issued.
          </div>
        )}
        ...
        <div className="space-y-1.5">
          ...
          />
          <p className={`text-xs ${exceedsCorporateLimit ? "text-destructive" : "text-muted-foreground"}`}>
            {exceedsCorporateLimit
              ? `Exceeds the Corporate Credit Limit. Up to ${formatCurrency(perCardMax)} allowed per card.`
              : `Maximum the card can spend. Up to ${formatCurrency(perCardMax)} per card (matches the Corporate Credit Limit).`}
          </p>
        </div>
        <div className="space-y-1.5">
      ...
      <DialogFooter>
        ...
        <Button onClick={submit} disabled={!firstTopUpDone || !requested || exceedsCorporateLimit || perTxnExceedsSpend || atmExceedsCap}>Issue card</Button>
      </DialogFooter>
    </DialogContent>
  );
}
...
function ManageCardDialog({ card }: { card: CardModel }) {
  ...
  const [spendLimit, setSpendLimit] = useState(card.spendLimit ? String(card.spendLimit) : "");
  ...
  // Card caps are not pooled — each card may be set up to the full Corporate
  // Credit Limit. If the Corporate Credit Limit later contracts, all card
  // caps are auto-reduced to match.
  const perCardMax = corporateDeposit;
  const exceedsCorporateLimit = newSpendLimit > perCardMax;
        ...
        <Tabs defaultValue="status" className="w-full">
          ...
          <TabsContent value="status" className="space-y-4 pt-4">
            ...
            <section className="space-y-3 rounded-lg border border-destructive/40 p-4">
              ...
              <p className="text-xs text-muted-foreground">
                ...
                be returned to the Corporate Account. This action cannot be undone.
              </p>
              <div className="space-y-1.5">
          ...
          <TabsContent value="limits" className="space-y-4 pt-4">
            <div className="space-y-1.5">
              ...
              <p className={`text-xs ${exceedsCorporateLimit ? "text-destructive" : "text-muted-foreground"}`}>
                {exceedsCorporateLimit
                  ? `Exceeds Corporate Credit Limit. Up to ${formatCurrency(perCardMax)} allowed per card.`
                  : `Maximum the card can spend. Up to ${formatCurrency(perCardMax)} per card (matches the Corporate Credit Limit). If the limit contracts after an unpaid bill, this limit is auto-reduced.`}
              </p>
            </div>
            <div className="space-y-1.5">
            ...
            <DialogFooter>
              ...
              <Button
                onClick={saveLimits}
                disabled={exceedsCorporateLimit || perTxnExceedsSpend || atmExceedsCap}
              >
                Save limits
              </Button>
            </DialogFooter>
          ...
          <TabsContent value="controls" className="space-y-6 pt-4">
            ...
            <section className="space-y-3">
              ...
              <p className="text-xs text-muted-foreground">
                ATM withdrawals are configured on the Limits tab — set a daily limit (max 20% of the daily spending limit) or leave blank to disable.
              </p>
            </section>
